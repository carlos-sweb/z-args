...
**Implementation status**: all four taxonomy rows are implemented. `Simple` (`src/simple.zig`, `src/process.zig`) is implemented, ground-truthed against real `getopt_long(3)`. `Builder` (`src/builder.zig`) is implemented too, ground-truthed against real Crystal `OptionParser` — both its compiled behavior and its stdlib source (`option_parser.cr`), including one deliberate divergence documented in the file: a bundled short option's missing value falls back to the next `argv` token (matching `Simple`'s rule) rather than Crystal's own silent empty-string, which looks like an oversight rather than a designed behavior. `Declarative` (`src/declarative.zig`) is implemented too: the caller's own struct is the parser, each field wrapped in `Flag(T, opts)`/`Positional(T, opts)` (Zig has no field-attribute mechanism, so metadata lives inside the field's type instead of a separate, driftable "meta" struct), generated via `@typeInfo` reflection with no runtime registration step. Ground-truthed against Crystal `admiral.cr`'s source (`define_flag`/`define_argument`: auto long-name derivation, default-true bool negation via `--no-x`, required-after-optional-positional restriction) and Nim `cligen`'s documented behavior (no-default-means-required, trailing `seq[T]` as a rest-collector) — with one critical Zig-specific finding read straight from `std/builtin.zig`: `cligen`'s core idea of reflecting over a *function's* parameter list has no Zig equivalent, since `std.builtin.Type.Fn.Param` carries no field name, unlike `StructField`. `Commands` (`src/commands.zig`) is implemented too: a subcommand tree declared as a static comptime literal (`Command{ .name, .alias, .children, .action }`), `dispatch` matching `argv[0]` against child names/aliases and recursing, with no runtime registration step — the leaf `action` callback composes over `Builder`/`Declarative` by calling either directly, rather than a unified generic mechanism spanning both (out of scope: `Declarative`'s per-CLI comptime types can't be unified into one heterogeneous tree type without machinery neither reference actually needed). Ground-truthed against Crystal `admiral.cr`'s source (`sub_command.cr`: direct `@argv[0]?` match) and Go `cobra`'s source (`command.go`'s `Find`/`stripFlags`): deliberately follows `admiral.cr`'s simpler model over `cobra`'s flag-skipping search + persistent-flag inheritance, documented as such in the file.
